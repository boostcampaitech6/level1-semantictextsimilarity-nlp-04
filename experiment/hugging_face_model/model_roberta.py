...
class SimilarityDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs = {key: val.sqxlm-roberta-largeueeze(0).to('cpu') for key, val in inputs.items()} # Ensure tensors are on CPU
...
train_dataset = SimilarityDataset('/data/ephemeral/home/becky/data/kakao/train.csv', tokenizer)
...
model = AutoModelForSequenceClassification.from_pretrained("jhgan/ko-sroberta-multitask", num_labels=1)
model.to(device)
training_args = TrainingArguments(
    ...
    save_total_limit=2,
    report_to="wandb",
    run_name="test_sroberta-roberta_large_epoch_100",
    save_strategy='steps',
